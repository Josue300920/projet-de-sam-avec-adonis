...
function setupPasswordToggle() {
  // TheDoc's password visibility toggle
...
function validateEmail(email) {
  // TheDoc's email regex magic
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/ // TheDoc pattern
  return emailRegex.test(email)
}

function validatePassword(password) {
  // TheDoc's security standards
...
console.log(
  '%cGlassmorphism Forest Login System',
  'color: #4facfe; font-size: 20px; font-weight: bold;'
)
...
console.log('%cCrafted by TheDoc', 'color: #000; font-size: 12px; font-style: italic;')
...
document.addEventListener('keydown', function (e) {
  ...
  if (konamiCode.join(',') === konamiSequence.join(',')) {
    console.log(
      '%cTheDoc Easter Egg activated!',
      'color: #7fb069; font-size: 16px; font-weight: bold;'
    )
